## API Report File for "@essex/test-lib-esm"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts

/// <reference types="react" />

// Warning: (ae-missing-release-tag) "add" is exported by the package, but it is missing a release tag (@alpha, @beta, @public, or @internal)
//
// @public (undocumented)
export function add(...nums: number[]): number;

// Warning: (ae-missing-release-tag) "b" is exported by the package, but it is missing a release tag (@alpha, @beta, @public, or @internal)
//
// @public (undocumented)
export const b = "b";

// Warning: (ae-missing-release-tag) "defaultExport" is exported by the package, but it is missing a release tag (@alpha, @beta, @public, or @internal)
//
// @public (undocumented)
const defaultExport = "defaultExportValue";
export default defaultExport;

// Warning: (ae-missing-release-tag) "HelloWorld" is exported by the package, but it is missing a release tag (@alpha, @beta, @public, or @internal)
//
// @public (undocumented)
export function HelloWorld(): JSX.Element;

// (No @packageDocumentation comment for this package)

```
